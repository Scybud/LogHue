...
import { checkIfEmpty, savedTaskDetails, renderExistingTasks, } from "../pages/personalTasks.js";
...
export function populateAssignDropdown(selectEl) {
    if (!selectEl || !loadedMembers || !Array.isArray(loadedMembers))
        return;
...
export function attachCreateApiKeyEvents(workspaceId) {
    ...
    `;
        // Copy button
        const copyApiKeyBtn = modal.querySelector("#copy-api-key-btn");
...
export async function attachAddMemberEvents(workspaceId) {
    ...
    const inviteByEmailBtn = document.getElementById("invite-email-btn");
    inviteByEmailBtn.onclick = () => {
        emailSection.style.display = "block";
        qrSection.style.display = "none";
    };
    // SWITCH TO QR MODE
    const inviteByQrCodeBtn = document.getElementById("invite-qr-btn");
...
export async function attachCreatePersonalTaskEvent(editingTaskId = null) {
    ...
    logTaskBtn.addEventListener("click", async (e) => {
        ...
        if (editingTaskId) {
            ...
            if (recurringValue)
                updatePayload.reminder_days = reminderDaysValue;
            ...
            const idx = savedTaskDetails.findIndex((t) => String(t.id) === String(editingTaskId));
            if (idx !== -1)
...
export async function insertTaskLogUpdate(supabase, workspaceId) {
    ...
    const taskIdEl = document.getElementById("taskLists");
    const taskId = taskIdEl.value;
    const statusEl = document.getElementById("taskLogUpdateStatus");
    const status = statusEl.value;
    const noteEl = document.getElementById("taskLogUpdateNote");
    ...
    //push notif
    const { pushNotifData, pushNotifError } = await supabase.functions.invoke("trigger-push", {
        body: {
            workspace_id: workspaceId,
            payload: {
...
//# sourceMappingURL=modalEvents.js.map