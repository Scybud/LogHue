...
export function attachCreateTaskEvent(workspaceId: string) {
  ...
  createTaskBtn.addEventListener("click", async () => {
    ...
    const taskTitleEl = document.getElementById(
      "taskTitle",
    ) as HTMLInputElement;
    const taskTitle = taskTitleEl.value.trim() || null;

    const taskDueDateEl = document.getElementById(
      "taskDueDate",
    ) as HTMLInputElement;
    ...
    const taskDescriptionEl = document.getElementById(
      "taskDescription",
    ) as HTMLInputElement;
...
export function attachCreateApiKeyEvents(workspaceId: string) {
  ...
  generateBtn.onclick = async () => {
    ...
    // Copy button
    const copyApiKeyBtn = modal.querySelector(
      "#copy-api-key-btn",
    ) as HTMLButtonElement;
...
export async function attachAddMemberEvents(workspaceId: string) {
  ...
  const inviteByEmailBtn = document.getElementById(
    "invite-email-btn",
  ) as HTMLButtonElement;
  ...
  // SWITCH TO QR MODE
  const inviteByQrCodeBtn = document.getElementById(
    "invite-qr-btn",
  ) as HTMLElement;
  ...
  // SEND EMAIL INVITE
  const sendEmailInviteBtn = document.getElementById(
  ...
  // GENERATE QR INVITE
  const generateQrBtn = document.getElementById(
    "generate-qr-btn",
  ) as HTMLButtonElement;
  ...
  // COPY INVITE LINK
  const copyInviteLinkBtn = document.getElementById(
  ...
  //SHARE INVITE LINK
  const shareInviteLinkBtn = document.getElementById(
...
export async function attachCreatePersonalTaskEvent() {
  ...
  const endTimeEl = document.getElementById("taskEndTime") as HTMLInputElement;
  const reminderDayEls = Array.from(
    document.querySelectorAll(".reminderDayCheck"),
  ) as HTMLInputElement[];
